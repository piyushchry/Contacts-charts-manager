...
//Define EditContact Component
const EditContact = () => {
  const dispatchFn = useDispatch()

  //fetch completeListObj from Redux store
  const completeListObj = useSelector(
    (rootReducer: any) => rootReducer.contactReducer
  );

  // Destructuring editContactData from completeListObj
  ...
  const submitEditForm = (event: { preventDefault: () => void }) => {
    ...
    //check first and last name are provided
    if (userFirstName !== '' && userLastName !== ''){
      //created newData object with updated details
      const newData = {
        id,
      ...
      alert("Please fill all details!!") //alert if any field is empty
    }
  };
  ...
  return (
    <div className="overflow-visible md:h-screen min-h-[580px] md:w-full flex flex-col md:text-center items-center justify-center border text-[#36454F] bg-[#F3F3F3]">
      <h1 className="text-[25px] md:text-3xl mb-8 font-bold text-[#050202]">
        Edit Contact
      </h1>
      <form onSubmit={submitEditForm}>
        ...
        <button
          className="hover:bg-[#00E8FF] hover:text-[#36454F] md:h-[50px] h-[40px] w-[200px] text-block font-bold rounded-xl drop-shadow-md bg-[#71ecbb] mt-5 "
          type="submit"
